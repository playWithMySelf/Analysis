...
angular.module("WebApp").controller('WorkListCtrl', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.load = function () {
        if (!$scope.start || !$scope.end || $scope.start.length === 0 || $scope.end.length === 0) {
            bootbox.dialog({title: "提示", message: "清选择时间段", buttons:{main: {label: " 关 闭 ", className: "dark icon-ban btn-outline"}}});
        } else {
            $http({
                url: "mvc/dispatch",
                params: {
                    controller: "StatisticController",
                    method: "queryWorkReport",
                    orgId: $scope.orgId,
                    start: $scope.start,
                    end: $scope.end
                }
